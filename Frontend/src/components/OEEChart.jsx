...
const OEEChart = ({ componentsData, oeeHistory, timestamps }) => {
  ...
  const barChartData = Object.entries(componentsData)
    ...
    .map(([name, data]) => {
      ...
      // Availability now calculated time-based in backend
      return {
        name: name.replace("-", " "),
  ...
  const hasBarData =
    barChartData.length > 0 && barChartData.some((d) => d.OEE > 0);

  // Prepare data for line chart (OEE trend over time)
  const lineChartData =
    oeeHistory && oeeHistory.length > 0
  ...
  const overallAvailability = calculateOverallMetric("availability_rate"); // Time-based from backend
      ...
      <div className="space-y-6">
        ...
        {lineChartData.length > 0 ? (
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 border border-slate-200">
            ...
            <div className="flex items-center justify-center h-64 text-slate-500">
              <p>
                Collecting trend data... Please wait for auto-refresh to
                populate the chart.
              </p>
            </div>
          </div>
